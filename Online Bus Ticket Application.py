...
def store():
    ...
    try:
        db=pymysql.connect(host="localhost",user="root",password="",database="busapp")
        insql="insert into tickets(PassengerName,Gender,Age,DateOfJourney,Class,StartingPoint,DestinationPoint)values('{}','{}','{}','{}','{}','{}','{}')".format(pn,ge,ag,dj,cl,sp,dp)
...
def update(rid):
    ...
    sp=request.form['StartingPoint']
    dp=request.form['DestinationPoint']
    #return pn+ge+ag+dj+cl+sp+dp
    try:
        db=pymysql.connect(host="localhost",user="root",password="",database="busapp")
        cu=db.cursor()
        sql="update tickets SET PassengerName='{}',Gender='{}',Age='{}',DateOfJourney='{}',Class='{}',StartingPoint='{}',DestinationPoint='{}' where PassengerId='{}'".format(pn,ge,ag,dj,cl,sp,dp,rid)
        cu.execute(sql)
        db.commit()
        return redirect('/')
    except Exception as e:
